(function(){
	...
	function jumpUrl2(href,type,channel,winning){//提交按钮type 信息 跳转页面
				var url;
				url = href+"carden_of_life_btn&type="+type;
				pushData(url,{
					id:+ new Date(),
					channel:channel
				});
				...
				var a;
				if(type == 2){
					a = dateUrl.QA + channel;
				}
				else if(channel != 1){//是否是微信
					channerType = "weibo0";
					if(type == 1){
						if(winning){
							a = dateUrl[channerType+type][1];
						}else{
							a = dateUrl[channerType+type][0];
						}
					}else{
						a = dateUrl[channerType+type];
					} 
				}else{
					channerType = "weixin0";
					a =  dateUrl[channerType+type];
	...
	var app = new Vue({
		...
		methods:{
			...
			jumpUrl:function(type){ //结果页按钮点击后执行跳转方法
				jumpUrl2(this.apiUrl,type,this.channel,this.winning);
			},
			openShare:function(){
				this.showShare = true;
			},
