...
QTTB = (function(){
	...
	function isCoinVersion(){
		if(bridge.hasNativeMethod("isCoinVersion")){
			...
			return !!window.qukanClient.isCoinVersion && window.qukanClient.isCoinVersion() 
		}else{
			return window.cpcAndroid && cpcAndroid.fetchInfo('isCoinVersion')
		}
	...
	function geToken(callback){
		...
		if(window.cpcAndroid){
		...
		}else if(bridge.hasNativeMethod("getCommonMsg")){
			...
			callback(window.qukanClient && window.qukanClient.getCommonMsg && window.qukanClient.getCommonMsg()) || ''
		}else{
	...
	var qb = {
		...
		getToken:geToken,
		isQTT: /qukan|clicash_android/.test(navigator.userAgent)
	}
