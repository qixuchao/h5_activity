...
const seajsRev = require("gulp-seajs-config-map");
  ...
  return (
    gulp
      ...
      .pipe(
        $.if(
          condition,
          $.rev.manifest(".tmp/rev-manifest.json", {
            base: ".tmp",
...
gulp.task("script", useEslint, () => {
  return (
    gulp
      ...
      .pipe($.if(condition, gulp.dest(".tmp")))
      .pipe($.if(condition,seajsRev({ base:'.tmp/js', configFile: "config.js" })))
      //.pipe($.if(condition, gulp.dest(".tmp/js")))
      .pipe($.if(condition,$.if(condition, gulp.dest(config.build.script))))
  );
});

// 注意： lib文件下不执行压缩
gulp.task("lib", useEslint, () => {
  return gulp.src(config.dev.lib).pipe(gulp.dest(config.build.lib));
});
